...
export function compareBenchmarks(...benchmarks) {
    ...
    for (const result of benchmarks) {
        console.log(`│ ${result.name.padEnd(12)} │ ${result.opsPerSecond.toFixed(0).padStart(8)} │ ${(result.avgTime * 1000).toFixed(2).padStart(9)} │ ${(result.minTime * 1000).toFixed(2).padStart(9)} │ ${(result.maxTime * 1000).toFixed(2).padStart(10)} │`);
    }
    console.log('└──────────────┴──────────┴───────────┴───────────┴────────────┘\n');
    if (benchmarks.length > 1) {
        const fastest = benchmarks.reduce((a, b) => a.opsPerSecond > b.opsPerSecond ? a : b);
...
export function runPerformanceTests() {
    ...
    results.push(benchmark('Signal Set', () => {
        const listeners = [];
        let value = 0;
        const set = (v) => { value = v; listeners.forEach(l => l()); };
        for (let i = 0; i < 100; i++)
            set(i);
    }, { iterations: 500 }));
    results.push(benchmark('VNode Create', () => {
        ...
        for (let i = 0; i < 100; i++) {
            h('div', { className: 'container' }, h('span', { id: 'item' }, 'text'), h('button', undefined, 'Click'));
        }
    }, { iterations: 500 }));
